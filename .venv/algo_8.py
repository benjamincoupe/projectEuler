...
def largestProductinaSeries(n):
    # Can also return the integers that resulted in largest product!
    pointer = 0
    product = 1
    while pointer < len(thousandDigits):
        new_product = 1
        integers = []
        for num in (thousandDigits[pointer:pointer+n]):
            new_product *= num
            integers += [num]
            if new_product > product:
                product = new_product
                best_integers = integers
        pointer += 1
    return product, best_integers
